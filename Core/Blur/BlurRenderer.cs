...
public sealed class BlurRenderer : IDisposable
{
    private const string Hlsl = """
        ...
        cbuffer Params : register(b0) { float2 texel; float2 dir; };
    ...
    public BlurRenderer(BlurDevice device, int width, int height)
    {
        ...
        _params = d.CreateBuffer(new BufferDescription(16, BindFlags.ConstantBuffer, ResourceUsage.Default));
    }
    ...
    public void Render(ID3D11Texture2D capturedFrame)
    {
        lock (_device.ContextLock)
        {
            ...
            // Upscale to the swapchain backbuffer and present.
            using (var backBuffer = _swapChain.GetBuffer<ID3D11Texture2D>(0))
            using (var rtv = _device.Device.CreateRenderTargetView(backBuffer))
            {
                DrawPass(_half[0].Srv, rtv, _width, _height, _psCopy, 0, 0);
            }
    ...
    {
        var ctx = _device.Context;
        var cb = new[] { 1f / width, 1f / height, dirX, dirY };
